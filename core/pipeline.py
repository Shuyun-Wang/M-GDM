...
class MGDMPipelineOutput(BaseOutput):
    videos: Union[torch.Tensor, np.ndarray]
    attn_map: torch.Tensor
    latent: torch.Tensor
    t: int
    mask: torch.Tensor
    pred_conditions: torch.Tensor


class MGDMPipeline(DiffusionPipeline):
    ...
    def prepare_image_latents(self, image, batch_size, num_images_per_prompt, device, do_classifier_free_guidance, generator=None):
        ...
        **kwargs,
        ...
        for i, t in enumerate(timesteps):
            ...
            for j, (latent, text_embedding, image_latent) in enumerate(zip(latent_model_input, text_embeddings, image_latents)):
                ...
                noise_pred,masks = self.unet(latent.unsqueeze(0), mvs.to(dtype=latents_dtype, device=device),frametype.to(dtype=latents_dtype, device=device), t, encoder_hidden_states=text_embedding.unsqueeze(0), img_condition=image_latent.unsqueeze(0))
        ...
        return MGDMPipelineOutput(videos=video, attn_map=None, latent=output_latent, t=output_t, mask=masks, pred_conditions=latents)
