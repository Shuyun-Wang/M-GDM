"""
MGDM Evaluation Script
Blind Bitstream-corrupted Video Recovery via Metadata-guided Diffusion Model
CVPR 2025

Requires ground-truth frames and masks. Computes PSNR, SSIM, LPIPS.
"""
import os
import argparse
...
from core.metrics import calc_psnr_and_ssim_per_mask
...
def run_evaluate(args, logger):
    ...
    loader = DataLoader(TestDataset(args), batch_size=1, shuffle=False, num_workers=1)
    totals = {"psnr": [], "ssim": [], "lpips": []}

    for items in loader:
        frames_PIL, corrupts, gt_masks, mvs, ft, video_name = items
        B, T = corrupts.shape[:2]
        vid = video_name[0]

        ori_frames   = [frames_PIL[i].squeeze().cpu().numpy() for i in range(T)]
        gt_masks     = gt_masks.to(device)
        corrupts     = corrupts.to(device)
        masks_metric = gt_masks[0, :, 0].cpu().numpy()
        ...
        for ori, comp, msk in zip(ori_frames, comp_frames, masks_metric):
            psnr, ssim = calc_psnr_and_ssim_per_mask(ori, comp, msk)
            if psnr != np.inf:
                cur_psnr.append(psnr)
                cur_ssim.append(ssim)
                lp_dev = next(lpips_model.parameters()).device
                cur_lpips.append(lpips_model(
                    torch.from_numpy(ori).permute(2,0,1).unsqueeze(0).to(lp_dev),
                    torch.from_numpy(comp).permute(2,0,1).unsqueeze(0).to(lp_dev),
                ).item())

        totals["psnr"].extend(cur_psnr)
        totals["ssim"].extend(cur_ssim)
        totals["lpips"].extend(cur_lpips)
        logger.info(f'{vid}  PSNR/SSIM/LPIPS: {sum(cur_psnr)/len(cur_psnr):.4f}/'
                    f'{sum(cur_ssim)/len(cur_ssim):.4f}/{sum(cur_lpips)/len(cur_lpips):.4f}')

    avg_psnr  = sum(totals["psnr"])  / len(totals["psnr"])
    avg_ssim  = sum(totals["ssim"])  / len(totals["ssim"])
    avg_lpips = sum(totals["lpips"]) / len(totals["lpips"])
    logger.info(f'Average PSNR/SSIM/LPIPS: {avg_psnr:.2f}/{avg_ssim:.4f}/{avg_lpips:.4f}')
    print(f'Average PSNR/SSIM/LPIPS: {avg_psnr:.2f}/{avg_ssim:.4f}/{avg_lpips:.4f}')


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument("--data_root",   type=str, default="datasets/DAVIS")
    ...
    parser.add_argument("--json",        type=str, default="test_davis.json")
    ...
    parser.add_argument("--alpha",       type=float, default=0.6, help="Blend weight: 1.0=diffusion only, 0.0=PRM only")
    ...
    logger = setup_logger(os.path.join(args.output_dir, f"evaluate_{ts}.log"))
