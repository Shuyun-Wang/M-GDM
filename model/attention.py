...
def create_1d_absolute_sin_cos_embedding(pos_len, dim):
    assert dim % 2 == 0
    i_matrix = 1 / torch.pow(10000, torch.arange(dim // 2, dtype=torch.float) / (dim / 2))
    i_matrix = i_matrix.to(torch.long)
    pos_vec = torch.arange(pos_len).to(torch.long)
    out = pos_vec[:, None] @ i_matrix[None, :]
    emb = torch.zeros(pos_len, dim, dtype=torch.float)
    emb[:, 0::2] = torch.sin(out)
    emb[:, 1::2] = torch.cos(out)
    return emb
...
class BasicTransformerBlock(nn.Module):
    ...
    def forward(self, hidden_states, encoder_hidden_states=None, timestep=None, attention_mask=None, video_length=None):
        ...
        if self.attn_temp:
            ...
            if video_length > 1 and is_xformers_available():
